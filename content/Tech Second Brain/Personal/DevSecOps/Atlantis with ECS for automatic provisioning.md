...
And I will try with `Infrastructure` as first, and `IaC` is first topic I relate to `GitOps`. So with `IaC`, you have multiple platform to hand on and one of most popular is `Atlantis`. You can choose alternative at [[Awesome Terraform#Tools|Alternative Atlantis]]
